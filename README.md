...
| 기능 | 설명 |
| --- | --- |
| 판서 | 펜 · 형광펜 · 지우개, 굵기 5단계, 바로가기 색상 6칸 |
| 도형 | 선 · 화살표 · 사각형 · 타원 · 표 · 텍스트, Shift 스냅(수평/수직/정비율) |
| 선택 | 필기내용 선택 후 이동 · 크기 조절 · 회전 · 삭제, 모니터 간 이동. 여러 개를 고르면 하나의 그룹으로 조작(모서리 4핸들 + 회전 1핸들), 휠로 크기 조절, 요소를 정확히 겨냥하지 않아도 잡힌다 |
...
| 트레이 | 트레이 아이콘으로 판서 켜기/끄기 · 툴바 보이기 · 업데이트 확인 · 설정 · 종료 |
| 툴바 설정 메뉴 | 툴바의 설정 버튼을 누르면 설정 · 도구 막대 닫기 · 프로그램 종료 메뉴가 뜬다 |
...
선택 도구로 무언가를 고른 동안에는 수식키 없는 아래 키가 **다른 앱보다 먼저** SS Pen에 잡힌다.
선택이 없거나 다른 도구일 때는 잡지 않으므로 평소 사용에는 영향이 없다.

| 키 | 동작 |
| --- | --- |
| `Esc` | 선택 해제 → 클릭 통과로 전환 |
| `Delete` / `Backspace` | 선택한 요소 일괄 삭제 |
| 마우스 휠 | 선택한 요소 크기 조절 (커서가 선택 안이면 커서 기준, 밖이면 중심 기준) |

선택 바깥을 제자리에서 한 번 클릭해도 해제되며, 해제되는 순간 클릭 통과가 켜진다.
...
# 통합 테스트 — 대화형 데스크톱이 필요하며 토폴로지는 동적으로 열거한다
dotnet test tests/SSPen.IntegrationTests/SSPen.IntegrationTests.csproj

# E2E 사용자 워크플로 테스트 — 실제 창을 띄워 화면을 점유한다
dotnet test tests/SSPen.E2ETests/SSPen.E2ETests.csproj
```

`dotnet test SSPen.sln` 은 통합·E2E 테스트까지 같이 돌리므로 일반 환경에서는 단위 테스트 프로젝트만 지정한다.
...
src/SSPen/
  ...
  Pin/          핀 고정 창 (클릭 통과 · 확대)
  Updates/      GitHub 릴리스 업데이트 확인·설치 (유일한 네트워크 코드)
  Diagnostics/  일자별 롤링 파일 로그
tests/          단위 테스트 · 머신 바인딩 통합 테스트 · E2E 사용자 워크플로 테스트
