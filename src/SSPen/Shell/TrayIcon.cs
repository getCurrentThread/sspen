...
public sealed class TrayIcon : IDisposable
{
    ...
    public TrayIcon(AppState state, Action openSettings, Action exitApp)
    {
        ...
        // 서피스는 WorkArea 크기라 작업표시줄을 덮지 않는다 — 이 메뉴가 떠 있는 동안의 ESC는
        // 메뉴의 것이다. SelectionKeyMonitor의 blocked 게이트가 이 플래그를 읽는다 (R3).
    ...
    /// <summary>트레이 컨텍스트 메뉴가 떠 있는가 — 그 동안의 ESC는 메뉴의 것이다 (R3).</summary>
    public bool IsMenuOpen { get; private set; }

    /// <summary>
    /// <see cref="IsMenuOpen"/>이 바뀌었다.
    ///
    /// <b>조건부 훅의 blocked 게이트에 들어가는 항은 '값 + 변경 이벤트'를 반드시 짝으로 갖는다.</b>
    /// 훅 안의 재판정은 '걸려 있는 훅이 삼키지 않게' 하는 방향만 구제할 뿐, 이미 내려간 훅을
    /// 되살리지 못한다. 계기가 없으면 메뉴가 떠 있는 동안 일어난 상태 변경 한 번이 훅을 내리고
    /// 메뉴를 닫아도 되살아나지 않아 ESC/Delete가 조용히 죽은 채 래치된다.
    /// </summary>
